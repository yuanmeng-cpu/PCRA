...
class DataManager:
            ...
            for j, each in enumerate(out):
                if self.index2word[each] == "<$>" :
                    ...
                    for idx, prob in sorted(enumerate(pred_ents[i][j]), key=lambda i: i[1], reverse=True):
                        if suc_flag or cnt >= topK or idx > 313:
                            break
    ...
    def get_recommendation_list(self,outputs,pred_ents):
        # print(self.id_nodename_map)
        outputs = outputs.cpu().numpy().tolist() #[64]
        #outputs:[6, 20, 15, 7, 4, 4, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3,
        # 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 8, 3, 3, 3] length=34
        pred_ents = pred_ents.cpu().numpy() #[64,length,315]  实体概率
        recommend_list = []
        recommend_list_index = []
        for i,out in enumerate(outputs): #64
            recommend_list_add = []
            recommend_list_index_add = []
            for j,each in enumerate(out): #length of sentence
                recommend_sublist = []  # 一个位置上的recommend_list
                recommend_sublist_index = []
                if self.index2word[each] == "<$>":  #<$>:entity token
                    k = 1
                    for idx, prob in sorted(enumerate(pred_ents[i][j]),key=lambda i: i[1], reverse=True):
                        if 0 <= idx <= 313 and k <= 1\
                                : #k表示前k个
                            nodename = self.id_nodename_map[idx]
                            recommend_sublist.append(nodename)
                            recommend_sublist_index.append(idx)
                            k += 1
                    recommend_list_add.append(recommend_sublist)
                    recommend_list_index_add.append(recommend_sublist_index)
            recommend_list.append(recommend_list_add)
            recommend_list_index.append(recommend_list_index_add)
        recommend_list_new  = []
        for i  in recommend_list:
            recommend_sublist_new = []
            if len(i) != 0:
                for j in i:
                    recommend_sublist_new.extend(j)
            recommend_list_new.append(recommend_sublist_new)

        return recommend_list_new,recommend_list_index

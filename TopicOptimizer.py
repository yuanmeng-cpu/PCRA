...
def sequence_cross_entropy_with_logits(logits: torch.FloatTensor,
    ...
    # print(log_probs_flat.device, targets_flat.device)
    negative_log_likelihood_flat = - torch.gather(log_probs_flat, dim=1, index=targets_flat.to(device=device))
    # shape : (batch, sequence_length)
    negative_log_likelihood = negative_log_likelihood_flat.view(*targets.size())
    # shape : (batch, sequence_length)
    negative_log_likelihood = negative_log_likelihood * weights.float().to(device=device)
    # shape : (batch_size,)
    per_batch_loss = negative_log_likelihood.sum(1) / (weights.sum(1).float().to(device=device) + 1e-13)
